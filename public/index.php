...
$app->post('/trajet/recherchetrajetparcompagnie', function (Request $request, Response $response) {
  //Recuperation des donnees venant du body de la requete
  $data = json_decode($request->getBody()->getContents(), true);
  // Verification des valeurs mises dans le body
  // Si les valeurs n'existe pas, nous assignons une valeur null
  $typ_voyage = $data["typ_voyage"] ?? null;
  $datedepart = $data["datealler"] ?? null;
  $datedestination = $data["dateretour"] ?? null;
  $nom = $data["nom"] ?? null;
  $depart = $data["depart"] ?? null;
  $destination = $data["destination"] ?? null;


  // Si l'utilisateur ne rempli pas les valeurs demande, nous affichons un
  // message d'erreur
  if (!$nom && !$depart && !$destination && !$datedepart && !$datedestination  ) {
    $data = array('message' => 'parametre invalide',);
    $payload = json_encode($data);
    $response->getBody()->write($payload);
    return $response->withStatus(400)
      ->withHeader('Content-Type', 'application/json');
  } else { // Sinon nous faisons notre requete sql

    /**
     * La requete sql suivant part rechercher tous les trajets ayant 
     * comme parametre destination, date, depart; type de voyage
     * gare et compagnie renseigne dans la request body
     * Pour faire un INNER JOIN, on specifie la table dans laquelle on veut faire le join
     * Ensuite on decide des parametres a comparer dans les deux tables
     * ! Attention, aux noms des tables
     */

    $sql = $typ_voyage == 'aller' ? "SELECT trajet.id_trajet,ville.nom as ville_depart,
           destination.nom as ville_destination, gare.nom as name_gare, compagnie.nom as compagnie_nom, trajet.datealler,dateretour, trajet.typ_voyage as type_de_voyage
          FROM trajet
          INNER JOIN gare
          ON trajet.id_gare = gare.id_gare 
          INNER JOIN compagnie
          ON gare.id_compagny = compagnie.id_compagny
          INNER JOIN ville
          ON trajet.depart = ville.id_ville 
          INNER JOIN ville as destination
          ON trajet.destination = destination.id_ville 
          WHERE trajet.destination=:destination
          AND trajet.depart=:depart
          AND gare.id_compagny=:nom
          AND trajet.datealler=:datealler
          AND trajet.dateretour=:dateretour
          AND trajet.typ_voyage=:typ_voyage
          " : "SELECT trajet.id_trajet,ville.nom as ville_depart,
           destination.nom as ville_destination, gare.nom as name_gare, compagnie.nom as compagnie_nom, trajet.datealler,dateretour, trajet.typ_voyage as type_de_voyage
          FROM trajet
          INNER JOIN gare
          ON trajet.id_gare = gare.id_gare 
    ...
    try {
      ...
      $stmt->bindParam(':datealler', $datealler);
      ...
      $stmt->execute();
      $trajets = $stmt->fetchAll(PDO::FETCH_OBJ);
      $encodeTrajet = json_encode($trajets);
      $response->getBody()->write($encodeTrajet);
      return $response
        ->withHeader('Content-Type', 'application/json');
    } catch (PDOException $e) {
      $error = array(
...
$app->post('/trajet/add', function (Request $request, Response $response, array $args) {
  ...
  $sql = "INSERT INTO trajet(typ_voyage,prix,datealler,dateretour,duree,depart,id_gare,heuredepart,heurearrive,destination)  VALUES (:typ_voyage,:prix,:datealler,:dateretour,:duree,:depart,:id_gare,:heuredepart,:heurearrive,:destination)";
...
// MODIFIER UN TRAJET
$app->put('/trajet/update/{id_trajet}',function (Request $request, Response $response, array $args) {
